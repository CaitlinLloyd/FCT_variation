...
def run():
    ...
    def get_rating(order_row, existed_file):
        ...
        for index, food in output_df.iterrows():
            ...
            corrected_rating = int(key[0][0])
            temp_list = (get_food_name(food_name),key[0], key[1])
            #temp_index = output_df[output_df['food_item'] == get_food_name(food_name)].index
            #print(temp_index)
            print(food_name)
            output_df.loc[index, 'food_item'] = get_food_name(food_name)
            output_df.loc[index, 'pref_recorded_response'] = key[0]
            output_df.loc[index, 'pref_rt'] = key[1]
            output_df.loc[index, 'pref_rating'] = corrected_rating
            output_df.loc[index, 'image_onset'] = image_onset
            output_df.loc[index, 'block'] = block_type
